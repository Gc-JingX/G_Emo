...
import com.omesoft.util.emojicon.emotionicon.VerticalImageSpan;
...
public final class EmojiconHandler {
	...
	static {
		...
		sEmojisMap.put(0x1f320, R.drawable.emoji_1f303); // TODO (rockerhieu)
		// review this emoji
		...
		sEmojisMap.put(0x1f508, R.drawable.emoji_1f508); // TODO (rockerhieu):
		// review this emoji
		...
		sEmojisMap.put(0x1f68b, R.drawable.emoji_1f68b); // TODO (rockerhieu)
		// review this emoji
	...
	public static void addEmojis(Context context, Spannable text, int emojiSize) {
		...
		for (int i = 0; i < length; i += skip) {
			...
			// Log.e("tkz", "::i::"+i+"::的值是::"+ c );
			// Log.e("tkz", " Character.codePointAt(text, i)::"+
			// Character.codePointAt(text, i));
			...
			if (icon == 0) {
				...
				if (icon == 0 && i + skip < length) {
					...
					if (followUnicode == 0x20e3) {
						...
						switch (unicode) {
							case 0x1f1ef:
								icon = (followUnicode == 0x1f1f5) ? R.drawable.emoji_1f1ef_1f1f5
										: 0;
								break;
							case 0x1f1fa:
								icon = (followUnicode == 0x1f1f8) ? R.drawable.emoji_1f1fa_1f1f8
										: 0;
								break;
							case 0x1f1eb:
								icon = (followUnicode == 0x1f1f7) ? R.drawable.emoji_1f1eb_1f1f7
										: 0;
								break;
							case 0x1f1e9:
								icon = (followUnicode == 0x1f1ea) ? R.drawable.emoji_1f1e9_1f1ea
										: 0;
								break;
							case 0x1f1ee:
								icon = (followUnicode == 0x1f1f9) ? R.drawable.emoji_1f1ee_1f1f9
										: 0;
								break;
							case 0x1f1ec:
								icon = (followUnicode == 0x1f1e7) ? R.drawable.emoji_1f1ec_1f1e7
										: 0;
								break;
							case 0x1f1ea:
								icon = (followUnicode == 0x1f1f8) ? R.drawable.emoji_1f1ea_1f1f8
										: 0;
								break;
							case 0x1f1f7:
								icon = (followUnicode == 0x1f1fa) ? R.drawable.emoji_1f1f7_1f1fa
										: 0;
								break;
							case 0x1f1e8:
								icon = (followUnicode == 0x1f1f3) ? R.drawable.emoji_1f1e8_1f1f3
										: 0;
								break;
							case 0x1f1f0:
								icon = (followUnicode == 0x1f1f7) ? R.drawable.emoji_1f1f0_1f1f7
										: 0;
								break;
							default:
								followSkip = 0;
			...
			if (icon > 0) {
				text.setSpan(new EmojiconSpan(context, icon, emojiSize), i, i
						+ skip, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
			}
		}
	 ...
	 * @param emojiSize
	 */
	public static void addEmojis2(Context context, Spannable text,
								  int emojiSize, int start, int lengthBefore, int lengthAfter) {
		...
		if (parser == null) {
			parser = new SmileyParser(context, emojiSize);
		}
		text = parser.replace2(text, emojiSize);
	}
	 ...
	 * @param @param context
	 * @param @param text
	 * @param @param emojiSize
	 * @param @param start
	 * @param @param lengthBefore
	 * @param @param lengthAfter 设定文件
	 * @return void 返回类型
	 * @throws
	 */
	public static void addEmojis3(Context context, Spannable text,
								  int emojiSize, int start, int lengthBefore, int lengthAfter) {

		String newtext = text.toString();

		// Editable oldtext = (Editable) mEditable.subSequence(0, start);
		// Editable mEditable = (Editable) text;
		// mEditable.subSequence(0, start);
		// text = mEditable;
		// Spannable text = mEditable;

		// if (lengthBefore > lengthAfter) {
		// text.
		// // text.setSpan(null, start, end, flags);
		// }

		int length = text.length();
		EmojiconSpan[] oldSpans = text.getSpans(0, length, EmojiconSpan.class);
		size = oldSpans.length;
		...
		for (int i = 0; i < length; i += skip) {
			...
			if (icon == 0) {
				...
				int unicode = Character.codePointAt(text, i);
				// Log.e("tkz", "::i::"+i+"::的值是::unicode"+ unicode );
				skip = Character.charCount(unicode);
		...
		VerticalImageSpan[] oldomeSpans = text.getSpans(0, length,
				VerticalImageSpan.class);
		size = oldomeSpans.length;
		for (int i = 0; i < size; i++) {
			// Log.e("newtkz", "oldomeSpans第几个：：：" + i);
			text.removeSpan(oldomeSpans[i]);
		}
		text = parser.replace2(text, emojiSize);
	}
}
